...
#ifndef __SPY_BUFFERS_STANDALONE_C
...
#ifdef ALLOC_PRAGMA
    #pragma alloc_text(INIT, SpyDriverEntry)
...
CONST FLT_OPERATION_REGISTRATION SpyCallbacks[] = {
    { IRP_MJ_CREATE,
      0,
...
const FLT_CONTEXT_REGISTRATION SpyContexts[] = {
...
CONST FLT_REGISTRATION SpyFilterRegistration = {
    ...
    SpyContexts,                               //  Context
    SpyCallbacks,                              //  Operation callbacks
...
#ifdef __SPY_BUFFERS_STANDALONE_C	

        //
        //  Now that our global configuration is complete, register with FltMgr.
        //

        status = SpyFltRegisterFilter( DriverObject,
                                    &FilterRegistration,
                                    &MiniSpyData.Filter );
...
#else 
    MiniSpyData.Filter = gFilterHandle;
#endif // __SPY_BUFFERS_STANDALONE_C	
...
--*/
{
    ...
    if ((InputBuffer != NULL) &&
        ...
        switch (command) {
            ...
            case GetMiniSpyProtectionFolder:
                {
                    ...
                    PDir = GetProttectinFolder();
                    pLogRecord = (PLOG_RECORD)OutputBuffer;

                    try {

                        pLogRecord->Length =  sizeof( LOG_RECORD ) + ROUND_TO_SIZE( PDir->Length + sizeof( UNICODE_NULL ), sizeof( PVOID ) );
                        //
                ...
                case SetMiniSpyProtectionFolder:
                {
                    PLOG_RECORD pLogRecord;
                    UNICODE_STRING Dir;
